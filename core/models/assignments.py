...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        if assignment.state == 'DRAFT':
            assertions.assert_valid(False, 'only submitted assignment can be graded')

        if auth_principal.teacher_id is not None:
            if assignment.state == 'GRADED':
                assertions.assert_valid(False, 'Assignment is already graded')
            if assignment.teacher_id != auth_principal.teacher_id:
                assertions.assert_valid(False, 'This assignment belongs to some other teacher')
